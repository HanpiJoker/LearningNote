...
int __init dm9000c_init(void)
{
	...
	iobase = (int)ioremap (0x20000000, 1024);    /* thisway.yjh@163.com */
	irq    = IRQ_EINT7;                          /* thisway.yjh@163.com */
	...
	/* BWSCON
	 * DW4[17:16]: 01 - 16bit(总共有16条数据线)
	 * WS4[18]   : 0  - WAIT DISABLE (DM9000C没有nWait口)
	...
	*bwscon = val;
	 ...
	 *            DM9000C的T1 >= 0ns, 所以它设为0;
	 * Tacc[10:8] : 读写信号的脉冲长度, DM9000C的T2 >= 10ns
	 *            所以它设为1, 表示2个hclk周期, hclk = 100MHz, 就是20ns;
	 * Tcoh[7:6]  : 当读信号nOE变为高电平后, 片选信号还要维持多长时间
	 *            DM9000C进行写操作时, nWE变为高电平之后, 数据线上还要维持最少3ns
	 *            DM9000C进行读操作时, nOE变为高电平之后, 数据线上的数据在6ns之内会消失
	 *            我们取一个宽松值: 让片选信号在nOE变为高电平后, 再维持10ns, 所以设为01;
	 * Tcah[5:4]  : 当片选信号变为高电平后, 地址信号还要维持多长时间
	 *            DM9000C的片选信号和CMD信号可以同时出现, 同时消失, 所以设为0;
	 * PMC [1:0]  : 00 - 正常模式
	 */
	*bankcon4 = (1<<8) | (1<<6);   
	/* 对于DM9000C可以设Tacc为1,
	 * 对于DM9000E, Tacc要设大一点, 比如最大值7,
	 * *bankcon4 = (7<<8) | (1<<6);
	 */
	iounmap (bwscon);
	iounmap (bankcon4);
...
void __exit dm9000c_exit(void)
{
	...
	free_netdev(dev);
#endif
	iounmap ((void *)iobase);               /* thisway.yjh@163.com */
