...
void Lcd_Port_Init(void)
{
    ...
    GPCCON  = 0xaaaaaaaa;    //GPIO用于VD[7:0], LCDVF[2:0], VM, VFRAME, VLINE, VCLK, LEND
...
void Tft_Lcd_Init(int type)
{
    ...
    case MODE_TFT_8BIT_240320:
        /*
         * 设置LCD控制气的控制寄存器LCDCON1~5
         * 1. LCDCON1:
         *      设置VCLK的频率：VCLK(Hz) = HCLK/[(CLKVAL+1)×2]
         *      选择LCD类型：TFT LCD
         *      设置显示模式： 8BPP
         *      先禁止LCD信号输出
         * 2. LCDCON2/3/4：
         ...
         * 现在，可以根据公式计算出显示器的频率：
         * 当HCLK=100mHz时，
         * Frame Rate = 1/[{(VSPPW+1)+(VBPD+1)+(LINEVAL+1)+(VFPD+1)}×
         *              {(HSPW+1)+(HBPD+1)+(HFPD+1)+(HOZVAL+1)}×
         *              {2×(CLKVAL+1)/(HCLK)}]
         *            = 60Hz
         * 3. LCDCON5:
         *      设置显示模式为8BPP时，调色板中的数据格式为：5:6:5
         *      设置HSYNC，VSYNC脉冲的极性(这需要参考具体LCD接口信号)：反转
         *      字节交换使能
         */
    ...
    case MODE_TFT_16BIT_240320:
         ...
         *            = 60Hz
         * 3. LCDCON5:
         *      设置显示模式为16BPP时，调色板中的数据格式为：5:6:5
         *      设置HSYNC，VSYNC脉冲的极性(这需要参考具体LCD接口信号)：反转
         *      半字(2字节)交换使能
         */
        LCDCON1 = (CLKVAL_TFT_240320<<8) | (LCDTYPE_TFT<<5) | \
         ...
         * 设置LCD控制器的地址寄存器LCDSADDR1~3
         * 帧内存与视口(view point)完全吻合，
         * 图像数据格式如下(16BPP时，帧缓冲区中的数据为调色板中的索引值)：
         *          |----PAGEWIDTH----|
         *   y/x    0   1   2       239
         *    0     rgb rgb rgb ... rgb
         *    1     rgb rgb rgb ... rgb
